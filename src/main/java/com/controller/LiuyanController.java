    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(liuyan !=null){
            ...
            BeanUtils.copyProperties( liuyan , view );//把实体数据重构到view中
                ...
                //级联表
                JiaolianEntity jiaolian = jiaolianService.selectById(liuyan.getJiaolianId());
                if(jiaolian != null){
    ...
    */
    ...
    public R save(@RequestBody LiuyanEntity liuyan, HttpServletRequest request){
        ...
        if(false)
            ...
            liuyan.setJiaolianId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<LiuyanEntity> queryWrapper = new EntityWrapper<LiuyanEntity>()
            .eq("yonghu_id", liuyan.getYonghuId())
            .eq("jiaolian_id", liuyan.getJiaolianId())
...
//                            liuyanEntity.setYonghuId(Integer.valueOf(data.get(0)));   //学员 要改的
//                            liuyanEntity.setJiaolianId(Integer.valueOf(data.get(0)));   //教练 要改的
